...
passwords.forEach(pswd => {
  const minimal = parseInt(pswd.substring(0, pswd.indexOf('-')));
  const maximum = parseInt(
    pswd.substring(pswd.indexOf('-') + 1, pswd.indexOf(' '))
  );
  const letter = pswd.substring(pswd.indexOf(' ') + 1, pswd.indexOf(':'));
  const appearances = pswd.split(letter).length - 2;
  if (appearances >= minimal && appearances <= maximum) {
    correctPswds = correctPswds + 1;
  }
});

console.log(correctPswds);
...
passwords.forEach(pswd => {
  ...
  const letter = pswd.substring(pswd.indexOf(' ') + 1, pswd.indexOf(':'));
  const first = pswd.indexOf(' ');
  const second = pswd.indexOf(' ', first + 1);
  const password = pswd.substring(second + 1, pswd.length);

  const occourances = [];

  occourances.push(password[minimal - 1]);
  occourances.push(password[maximum - 1]);

  let nr = 0;

  occourances.forEach(oc => {
    if (oc === letter) {
      nr = nr + 1;
    }
  });
