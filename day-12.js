...
const smallBatch = ['F10', 'N3', 'F7', 'R90', 'F11'];
const coordinates = {
  N: 0,
  E: 0,
  W: 0,
...
instructions.forEach(instruction => {
  ...
  if (direction === 'F') {
    coordinates[orientation] = coordinates[orientation] + value;
  } else if (direction === 'R') {
    const change = value / 90;
    let index = compas.indexOf(orientation) + change;

    if (index > 3) {
      index = index - 4;
    }
    orientation = compas[index];
  } else if (direction === 'L') {
    const change = value / 90;
    let index = compas.indexOf(orientation) - change;
    ...
    coordinates[direction] = coordinates[direction] + value;
  }
});
